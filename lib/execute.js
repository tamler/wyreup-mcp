...
async function executeTool(toolConfig, requestPayload, originalHeaders, { DEBUG }) { // Removed toolsBaseUrl
    ...
    // Process tool-specific authentication
    if (toolConfig.auth) {
        if (DEBUG) console.log(chalk.blueBright(`[DEBUG]   Processing auth type: ${toolConfig.auth.type}`));
        switch (toolConfig.auth.type) {
            case 'header':
                if (toolConfig.auth.name && typeof toolConfig.auth.value === 'string') { // Ensure value is string, even if empty
                    headersToForward[toolConfig.auth.name] = toolConfig.auth.value;
                } else if (DEBUG) {
                    console.warn(chalk.yellowBright(`[DEBUG] Auth type 'header' for tool ${toolConfig.name} is missing name or value.`));
                }
                break;
            case 'basic':
                if (toolConfig.auth.username && typeof toolConfig.auth.password === 'string') { // Ensure password is string
                    const credentials = `${toolConfig.auth.username}:${toolConfig.auth.password}`;
                    headersToForward['Authorization'] = `Basic ${Buffer.from(credentials).toString('base64')}`;
                } else if (DEBUG) {
            ...
            case 'jwt':
                if (toolConfig.auth.token) {
                    headersToForward['Authorization'] = `Bearer ${toolConfig.auth.token}`;
                } else if (DEBUG) {
    ...
    if (DEBUG) {
        console.log(chalk.blueBright(`[DEBUG]   Final Forwarding Headers: ${JSON.stringify(headersToForward, null, 2)}`));
    }
