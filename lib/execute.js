...
async function executeTool(
  ...
  if (toolConfig.auth) {
    if (DEBUG)
      console.log(
        chalk.blueBright(
          `[DEBUG]   Processing auth type: ${toolConfig.auth.type}`
        )
      )
    switch (toolConfig.auth.type) {
      case 'header':
        if (toolConfig.auth.name && typeof toolConfig.auth.value === 'string') {
          ...
          console.warn(
            chalk.yellowBright(
              `[DEBUG] Auth type 'header' for tool ${toolConfig.name} is missing name or value.`
            )
          )
        }
        break
      case 'jwt':
        if (toolConfig.auth.token) {
          ...
          console.warn(
            chalk.yellowBright(
              `[DEBUG] Auth type 'jwt' for tool ${toolConfig.name} is missing token.`
            )
          )
        }
        break
      default:
        if (DEBUG)
          console.warn(
            chalk.yellowBright(
              `[DEBUG] Unknown auth type: ${toolConfig.auth.type} for tool ${toolConfig.name}`
            )
          )
    }
  }
  ...
  try {
    ...
    // Attempt to parse as JSON, fallback to text if not JSON
    let responseData
