...
function validateManifest(config, filePath, { VALIDATE_FLAG, SERVE_FLAG, shouldStartServer }) {
  ...
  if (!Array.isArray(config.tools)) {
    ...
    config.tools.forEach((tool, index) => {
      ...
      if (tool.hasOwnProperty('auth')) { // Check if auth property exists
        ...
        if (typeof tool.auth !== 'object' || tool.auth === null) {
          ...
          const validTypes = ['header', 'basic', 'jwt'];
          if (typeof tool.auth.type !== 'string' || !tool.auth.type.trim()) {
            ...
            switch (tool.auth.type) {
              case 'header':
                ...
                break;
              case 'basic':
                if (typeof tool.auth.username !== 'string' || !tool.auth.username.trim()) {
                  errors.push(chalk.red(`Tool ${toolIdentifier}: Field "auth.username": Required and must be a non-empty string for basic auth. Path: ${toolAuthPath}.username`));
                }
                if (typeof tool.auth.password !== 'string' || !tool.auth.password.trim()) { // Password can be empty, but must be a string if provided
                  errors.push(chalk.red(`Tool ${toolIdentifier}: Field "auth.password": Required and must be a string for basic auth. Path: ${toolAuthPath}.password`));
                }
                break;
              case 'jwt':
